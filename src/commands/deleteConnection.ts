...
export class deleteConnectionCommand extends BaseCommand {
  async run(connectionNode: ConnectionNode) {
    ...
    if (connectionNode) {
      await deleteConnectionCommand.deleteConnection(
        connections,
        connectionNode.id,
      );
      return;
    ...
    const hostToDelete = await vscode.window.showQuickPick(hosts, {
    ...
    await deleteConnectionCommand.deleteConnection(
      connections,
      hostToDelete.connection_key,
    );
  }

  private static async deleteConnection(
