...
export class selectTopCommand extends BaseCommand {
  async run(
    ...
    if (count === 0) {
      // prompt for count
      const countInput: string = await vscode.window.showInputBox({
        prompt: 'Select how many rows?',
        placeHolder: 'limit',
      });
      ...
      if (Number.isNaN(count)) {
        vscode.window.showErrorMessage(
          'Invalid quantity for selection - should be a number',
        );
        return;
      }
    }
    ...
    if (withNames) {
      ...
      const configSort = Global.Configuration.get<string>(
      ...
      try {
        ...
        columnsToSelect = res.rows.map<string>((column) =>
          Database.getQuotedIdent(column.column_name),
        );
      } catch (err) {
        return err;
      } finally {
